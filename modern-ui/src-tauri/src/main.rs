...
fn gerar_relatorio_pendencias(
    ...
    let total: usize = input.secoes.iter().map(|s| s.linhas.len()).sum();
    let mut doc = DocumentoDocx::new();
    doc.titulo_ata(&input.titulo);
    ...
    if !input.criterio.trim().is_empty() {
        doc.paragrafo(&input.criterio);
    }

    if input.secoes.is_empty() {
        doc.caixa_aviso("Nenhuma pendência encontrada. Tudo entregue!");
    } else {
        for (indice, secao) in input.secoes.iter().enumerate() {
            if indice > 0 {
                doc.paragrafo("");
            }
            doc.paragrafo_negrito(&secao.titulo);
            let mut linhas = vec![vec![
                CelulaDocx::cabecalho(&input.coluna_item),
                CelulaDocx::cabecalho("Bimestres em falta"),
            ]];
    ...
    let pasta = data_dir()
        ...
        .join("pendencias");
    fs::create_dir_all(&pasta).map_err(|e| e.to_string())?;
    let escopo = if input.escopo.trim().is_empty() {
        "pendencias".to_string()
    } else {
        sanitizar_segmento(&input.escopo)
    };
    let arquivo = pasta.join(format!(
        "pendencias_{}_{}.docx",
        escopo,
