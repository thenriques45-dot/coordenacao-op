 ...
 *   criarFormulario de novo atualiza o mesmo Forms (as respostas já
 *   recebidas são preservadas; as perguntas são recriadas)
...
const TITULO_FORMULARIO = "Planejamento Docente — Ensino Médio";
...
const PROP_ETAPA     = "PLANEJAMENTO_EM_ETAPA";
const PROP_INDICE    = "PLANEJAMENTO_EM_INDICE";
const PROP_FORM_ID   = "PLANEJAMENTO_EM_FORM_ID";
const PROP_NAVEGACAO = "PLANEJAMENTO_EM_NAVEGACAO";

const FUNCAO_PRINCIPAL = "criarFormulario";
...
const ETAPAS = [
  ...
  "limpeza_exclusao",
  "criacao_base",
].concat(SERIES.map(function (serie, indice) {
  return "criacao_serie_" + indice;
})).concat([
  "criacao_final",
]);
...
function resetarCriacaoFormulario() {
  ...
  props.deleteProperty(PROP_NAVEGACAO);
  removerGatilhosContinuacao_();
  Logger.log("Progresso e acionadores limpos. Execute criarFormulario para recomeçar.");
}

// Faz o script esquecer o Forms memorizado: a próxima execução de
// criarFormulario criará um formulário novo do zero.
...
function executarEtapa_(form, etapa, props, inicio) {
  ...
  if (etapa.indexOf("criacao_serie_") === 0) {
    var serie = SERIES[parseInt(etapa.slice("criacao_serie_".length), 10)];
    return criarBlocoSerie_(form, props, serie, restaTempo);
  }
...
function excluirItens_(form, restaTempo) {
  ...
  if (sobras > 0) {
    ...
    throw new Error(
      ...
      "Verifique as permissões e execute criarFormulario novamente."
    );
  }
  return true;
}
...
// ─── CRIAÇÃO ────────────────────────────────────────────────────────────────
// A ligação entre as etapas (IDs do item de Série, das páginas de cada série
// e das páginas "folha") fica salva em PROP_NAVEGACAO, porque a navegação só
...
function lerNavegacao_(props) {
  ...
  return bruto
    ? JSON.parse(bruto)
    : { serieItemId: 0, paginasSerie: {}, paginasFinais: [] };
}
...
// SEÇÃO 1 — IDENTIFICAÇÃO (e a pergunta de Série, que ramifica o restante)
function criarBase_(form, props, restaTempo) {
  ...
  var itemSerie = form.addListItem()
    ...
    .setRequired(true);
  itemSerie.setChoiceValues(SERIES); // provisório; a navegação é ligada na etapa final

  salvarNavegacao_(props, {
    serieItemId: itemSerie.getId(),
    paginasSerie: {},
    paginasFinais: [],
  });
  return true;
...
// as páginas por série e componente:
//   • Série: leva à página da série (Turma + Componente).
//   • Turma: checkbox simples, não ramifica.
//   • Componente: só são oferecidos, para a série escolhida, os componentes
//     com Escopo-Sequência real para ela (componentesValidosDaSerie_), mais
//     os sem Currículo Priorizado (texto livre) e os ainda sem dado algum.
//   • Bimestre: só os bimestres com aulas reais. Se houver um único bimestre
//     válido, a pergunta nem aparece.
...
//
// Os títulos embutem componente e série para que cada pergunta vire uma
// coluna inequívoca na planilha de respostas.
function criarBlocoSerie_(form, props, serie, restaTempo) {
  if (!restaTempo()) return false;
  ...
  var itemComponente = form.addListItem()
    .setTitle("Componente curricular — " + serie)
  ...
  var escolhasComponente = componentesValidos.map(function (componente) {
    ...
    construirBlocoBimestreEscopo_(form, serie, componente, paginaComponente, paginasFinais);
    return itemComponente.createChoice(componente, paginaComponente);
  });
  itemComponente.setChoices(escolhasComponente);

  navegacao.paginasSerie[serie] = paginaSerie.getId();
...
function construirBlocoBimestreEscopo_(form, serie, componente, paginaAtual, paginasFinais) {
  var escopo = ESCOPOS_POR_COMPONENTE[componente];
  ...
  if (semCurriculo || bimestresValidos.length === 0) {
    form.addListItem()
      .setTitle("Bimestre — " + componente + " — " + serie)
      .setRequired(true)
      .setChoiceValues(BIMESTRES);
    form.addParagraphTextItem()
      .setTitle("Aulas e objetivos planejados — " + componente + " — " + serie)
      .setHelpText(
        semCurriculo
...
function criarFinalENavegacao_(form, props, restaTempo) {
  ...
  //   • cada série da pergunta inicial leva à sua página de Turma/Componente;
  ...
  var itemSerie = itensPorId[navegacao.serieItemId].asListItem();
  itemSerie.setChoices(SERIES.map(function (serie) {
    return itemSerie.createChoice(
      serie,
      itensPorId[navegacao.paginasSerie[serie]].asPageBreakItem()
    );
  }));
